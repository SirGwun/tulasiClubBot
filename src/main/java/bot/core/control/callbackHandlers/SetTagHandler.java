...
public class SetTagHandler implements CallbackHandler {
    private final Callbacks action = Callbacks.setTag;
    @Override
    ...
    public boolean match(Update update) {
        return update.hasCallbackQuery() && update.getCallbackQuery().getData().startsWith(Callbacks.setTag + "_");
    }
    ...
    public boolean isFormatCorrect(String callback) {
        ...
        if (!data[0].equalsIgnoreCase(Callbacks.setTag.toString())) return false;
        try {
